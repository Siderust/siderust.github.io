...
const repoRequests = new Map<string, Promise<GitHubRepo | null>>();
const releaseRequests = new Map<string, Promise<GitHubRelease | null>>();
const readmeRequests = new Map<string, Promise<string | null>>();

let githubApiDisabledReason: string | null = null;
...
function disableGitHubApi(reason: string): void {
  if (githubApiDisabledReason) {
    return;
  }

  githubApiDisabledReason = reason;
  console.warn(`Disabling GitHub API fetches for this build: ${reason}`);
}

function cacheRateLimitFailure(repo: string, status: number): null {
  disableGitHubApi(`rate limit or access failure (HTTP ${status})`);
...
async function fetchRepo(repo: string): Promise<GitHubRepo | null> {
  ...
  if (repoRequests.has(repo)) {
    return repoRequests.get(repo) ?? null;
  }

  const request = (async () => {
    try {
      const response = await fetch(`https://api.github.com/repos/${siteConfig.org}/${repo}`, {
        headers: getGitHubHeaders(),
      });

      if (!response.ok) {
        if (response.status === 403 || response.status === 429) {
          return cacheRateLimitFailure(repo, response.status);
        }
...
async function fetchLatestRelease(repo: string): Promise<GitHubRelease | null> {
  ...
  if (releaseRequests.has(repo)) {
    return releaseRequests.get(repo) ?? null;
  }
  ...
  const request = (async () => {
    try {
      ...
      if (!response.ok) {
        if (response.status === 403 || response.status === 429) {
          disableGitHubApi(`rate limit or access failure (HTTP ${response.status})`);
        }
        releaseCache.set(repo, null);
        return null;
      }
      ...
      releaseRequests.delete(repo);
    }
  })();

  releaseRequests.set(repo, request);
  return request;
}
...
async function fetchReadme(repo: string): Promise<string | null> {
  ...
  if (readmeRequests.has(repo)) {
    return readmeRequests.get(repo) ?? null;
  }
  ...
  const request = (async () => {
    try {
      ...
      readmeRequests.delete(repo);
    }
  })();

  readmeRequests.set(repo, request);
  return request;
}
...
export function formatDate(dateString: string, locale: Locale = defaultLocale): string {
  ...
  const bcp47: Record<string, string> = { en: 'en-US', ca: 'ca' };
 ...
 */
export function formatRelativeTime(dateString: string, locale: Locale = defaultLocale): string {
  const date = new Date(dateString);
  const now = new Date();
  const diffMs = now.getTime() - date.getTime();
  const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));

  if (diffDays === 0) return t(locale, 'time.today');
  if (diffDays === 1) return t(locale, 'time.yesterday');
  if (diffDays < 7) return t(locale, 'time.daysAgo', { count: diffDays });
  if (diffDays < 30) return t(locale, 'time.weeksAgo', { count: Math.floor(diffDays / 7) });
  if (diffDays < 365) return t(locale, 'time.monthsAgo', { count: Math.floor(diffDays / 30) });
  return t(locale, 'time.yearsAgo', { count: Math.floor(diffDays / 365) });
}
...
/**
 * Gets status label for display (locale-aware)
 */
export function getStatusLabel(status: RepoMetadata['status'], locale: Locale = defaultLocale): string {
